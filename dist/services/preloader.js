...
function inferLanguageFromUrl(url) {
    const match = url.match(/[_-]([a-z]{2}(?:-[A-Z]{2})?)[_.]/) ||
...
function extractTracksFromCaptionArray(captions) {
    ...
    for (const item of captions) {
        if (!item || typeof item !== 'object')
            continue;
        const obj = item;
        const url = toStringIfPresent(obj.url) ||
        ...
        if (!url || !url.includes('.vtt'))
            continue;
        const languageRaw = toStringIfPresent(obj.language) ||
...
function collectVttUrlsRecursively(data, maxNodes = 2000) {
    const tracks = [];
...
function extractCaptionTracks(data) {
    ...
    const root = data;
    const arrays = [];
    if (Array.isArray(root?.asset?.captions))
        arrays.push(root.asset.captions);
    if (Array.isArray(root?.asset?.caption_tracks))
        arrays.push(root.asset.caption_tracks);
    if (Array.isArray(root?.captions))
        arrays.push(root.captions);
    if (Array.isArray(root?.results))
        arrays.push(root.results);
    for (const arr of arrays)
        tracks.push(...extractTracksFromCaptionArray(arr));
...
async function fetchLectureCaptionTracks(courseId, lectureId, signal) {
    ...
    if (numericCourseId) {
        attempts.push(async () => {
            const url = `https://www.udemy.com/api-2.0/users/me/subscribed-courses/${numericCourseId}/lectures/${lectureId}/` +
    ...
    attempts.push(async () => {
        const url = `https://www.udemy.com/api-2.0/lectures/${lectureId}/` +
            `?fields[lecture]=title,asset&fields[asset]=captions`;
        const data = (await fetchJson(url, signal));
        const tracks = extractCaptionTracks(data);
        return { lectureTitle: typeof data?.title === 'string' ? data.title : undefined, tracks };
    ...
    if (numericCourseId) {
        attempts.push(async () => {
            const url = `https://www.udemy.com/api-2.0/courses/${numericCourseId}/subscriber-curriculum-items/` +
 ...
 */
export async function preloadLecture(request) {
