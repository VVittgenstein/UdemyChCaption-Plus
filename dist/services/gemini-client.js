...
export async function generateContent(options) {
    const { apiKey, model, systemInstruction, contents, temperature = 0.3, maxOutputTokens, timeout = DEFAULT_TIMEOUT, stream = true, signal, } = options;
    // Validate inputs
    if (!apiKey) {
...
export async function validateApiKey(apiKey) {
    try {
        const response = await fetch(`${GEMINI_API_BASE}/models?key=${apiKey}`, { method: 'GET' });
        if (!response.ok) {
 ...
 */
export function convertFromOpenAIFormat(messages) {
